...
class FirestoreDbService(private val db: FirebaseFirestore) {
    ...
    private val addToFirestoreAction: (DocumentReference.(String) -> Task<*>) = { eventId: String ->
        ...
        collection(net.squanchy.service.firebase.FirestoreDbService.Companion.COLLECTION_FAVORITES).document(eventId).set(firestoreFavorite)
    }
    ...
    private val deleteFromFirestoreAction: (DocumentReference.(String) -> Task<*>) = { eventId ->
        collection(net.squanchy.service.firebase.FirestoreDbService.Companion.COLLECTION_FAVORITES).document(eventId).delete()
    }
...
private inline fun <reified T> ObservableEmitter<T>.emitSnapshotResult(snapshot: DocumentSnapshot?, exception: Exception?) {
    when {
        ...
        else -> onError(net.squanchy.service.firebase.NonExistingDocumentException(snapshot))
    }
}
...
private inline fun <reified T> ObservableEmitter<T>.emitExistingSnapshotValue(snapshot: DocumentSnapshot) {
    try {
        ...
        if (modelObject == null) {
            onError(net.squanchy.service.firebase.NonExistingDocumentException(snapshot))
        } else {
            onNext(modelObject)
        }
    } catch (e: RuntimeException) {
